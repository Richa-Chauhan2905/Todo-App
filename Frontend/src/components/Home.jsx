    ...
    return (
        <div className='my-10 bg-gray-100 max-w-lg lg:max-w-xl rounded-lg shadow-lg mx-8 sm:mx-auto p-6'>
            ...
            <div className='mb-4 flex'>
                ...
                <button
                    onClick={todoCreate}
                    className='bg-blue-600 border rounded-r-md text-white px-4 py-2 hover:bg-blue-900 delay-200'>
                    Add
                </button>
            </div>
            {loading ? (
                    ...
                    {todos.map((todo, index) => (
                        <li key={todo._id || index} className='flex items-center justify-between p-3 bg-gray-100 rounded-md'>
                            <div className="flex items-center">
                                <input
                                    type="checkbox"
                                    checked={todo.completed} onChange={() => todoStatus(todo._id)} className="mr-2" />
                                <span className={`${todo.completed ? "line-through text-gray-500 font-semibold" : ""}`}>{todo.text}</span>
                            </div>
